/**
 * Full-text search composable — wraps C# streaming search backend.
 *
 ...
 *
 * bookId and bookTitle arrive pre-populated from C# in every result batch.
 * tocText may arrive empty when the index was built in minimal mode (no stored metadata).
 * In that case, onBatch runs GET_TOC_PATHS_FOR_LINES to fill tocText from the DB.
 */
import { ref } from 'vue'
...
// Tracks in-flight onBatch promises so onComplete waits for all batches to finish.
...
export function useFullTextSearch(isIndexing?: () => boolean) {
  ...
  let _startPending = false  // prevents concurrent executeSearch calls while FtsSearchStart is in-flight
  ...
  async function _startStream(
    ...
    _searchListeners.set(searchId, {
      onBatch: async (batch) => {
        ...
        // tocText may be empty when the index was built in minimal mode.
        // Enrich any results missing tocText with a single DB query before appending.
        const missingToc = batch.filter((r) => !r.tocText)
        if (missingToc.length > 0) {
          try {
            const lineIds = missingToc.map((r) => r.lineId)
            ...
            /* non-fatal — tocText stays empty if the query fails */
          }
        }
        results.value = [...results.value, ...batch]
        // Resolve after appending so the scroller renders with actual data.
        resultsReadyResolve?.()
        try {
  ...
  // Cache key includes all options that affect results so variants are stored separately.
  ...
  async function executeSearch(q: string) {
    if (!q.trim()) return
    if (_startPending) return  // drop the call — a search is already starting
