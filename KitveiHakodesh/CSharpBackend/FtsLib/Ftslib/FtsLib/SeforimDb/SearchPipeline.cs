...
namespace FtsLib.SeforimDb
{
    ...
    internal static class SearchPipeline
    {
        ...
        internal static IEnumerable<SearchResult> Search(
            ...
            string            indexPath,
            string            dbPath,
            List<(string dat, string db, int segId)> livePaths,
            SearchLease       lease,
            int               cap = 0,
            ...
            out bool          hardMiss)
        {
            ...
            foreach (var alt in group.Alternatives)
            {
                ...
                {
                    // '*' overrides '%' — grammar flags are already cleared by the parser
                    // when '*' is present.  '?' is handled inside HebrewWildcardExpander.
                    expanded = reader.ExpandWildcard(alt.Pattern);
                    if (expanded.Count == 0) continue;
                }
                else if (alt.IsGrammar)
                {
                    // Grammar expansion — compatible with '?':
                    // If the pattern contains '?', first unroll optional-char variants
                    // via HebrewWildcardExpander (which handles '?' without '*'), then
                    // apply grammar expansion to each resulting base word.
                    if (alt.Pattern.IndexOf('?') >= 0)
                    {
                        // Use ExpandStar with no '*' — HebrewWildcardExpander.Expand
                        // handles '?'-only patterns correctly (no LIKE query, just
                        // optional-char unrolling + literal lookup).
                        var baseWords = reader.ExpandWildcard(alt.Pattern);
                        expanded = new List<string>();
                        foreach (var baseWord in baseWords)
                        {
                            ct.ThrowIfCancellationRequested();
                            var grammarHits = reader.ExpandGrammar(baseWord, alt.GrammarPrefix, alt.GrammarSuffix);
                            foreach (var hit in grammarHits)
                                expanded.Add(hit);
                        }
                    }
                    else
                    {
                        expanded = reader.ExpandGrammar(alt.Pattern, alt.GrammarPrefix, alt.GrammarSuffix);
                    }
