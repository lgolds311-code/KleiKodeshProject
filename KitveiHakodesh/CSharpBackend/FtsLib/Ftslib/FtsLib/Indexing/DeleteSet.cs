...
namespace FtsLib.Indexing
{
    ...
    /// deleted from one segment.
    ///
    /// Each segment owns its own DeleteSet, persisted as a per-segment
    /// <c>seg_L_ID.del</c> file (sorted varint-delta, same codec as posting lists).
    ///
    /// Loaded by IndexReader (to filter search results for that segment only) and
    /// by SegmentMerger (to purge deleted IDs permanently when the segment is merged).
    ///
