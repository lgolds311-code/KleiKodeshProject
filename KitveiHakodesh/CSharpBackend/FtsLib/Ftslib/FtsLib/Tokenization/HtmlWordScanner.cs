    ...
    internal abstract class HtmlWordScanner
    {
        ...
        protected void Scan(string text)
        {
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                // ── GERESH / GERSHAYIM ─ stripped silently so that Hebrew
                //    abbreviations like רשב"א are indexed as a single token ──
                // U+05F3 ׳ (geresh), U+05F4 ״ (gershayim), U+0022 " (ASCII double-quote
                // typed by users who lack the Unicode punctuation on their keyboard).
                if (c == '\u05F3' || c == '\u05F4' || c == '"')
                    continue;
