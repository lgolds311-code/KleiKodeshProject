...
namespace FtsLibTest
{
    ...
    internal static class PerformanceTest
    {
        ...
        private static readonly PerfCase[] Cases =
        {
            ...
            // ── Grammar expansion (%) ─────────────────────────────────
            new PerfCase("grammar / prefix only (%word)",
                "%תורה"),

            new PerfCase("grammar / suffix only (word%)",
                "תורה%"),

            new PerfCase("grammar / both (%word%)",
                "%תורה%"),

            new PerfCase("grammar / prefix AND literal",
                "%ישראל כי"),

            new PerfCase("grammar / both AND literal",
                "%תורה% מצוה"),

            new PerfCase("grammar / optional char + prefix (%word? )",
                "%שלו?ם"),

            new PerfCase("grammar / ids only — prefix",
                "%תורה",
                idsOnly: true),
        };
            ...
            PerfCase     c)
        {
            ...
            foreach (var group in parsed.Groups)
            {
                ...
                else if (group.IsGrammar)
                    GrammarExpander.Expand(group.Pattern, group.GrammarPrefix, group.GrammarSuffix, segments);
            }
            swA.Stop();
