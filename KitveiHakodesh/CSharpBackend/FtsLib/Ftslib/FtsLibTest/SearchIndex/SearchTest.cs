...
namespace FtsLibTest
{
    ...
    internal static class SearchTest
    {
        ...
        private static readonly QueryCase[] Queries =
        {
            ...
            // ── Grammar expansion (%) ─────────────────────────────────
            // prefix only: %תורה → בתורה, לתורה, מתורה, שתורה, התורה, …
            new QueryCase("%תורה",
                QueryKind.Grammar,
                minResults: 1),

            // suffix only: תורה% → תורתי, תורתך, תורתו, תורתנו, …
            new QueryCase("תורה%",
                QueryKind.Grammar,
                minResults: 1),

            // both: %תורה% → full prefix+suffix expansion
            new QueryCase("%תורה%",
                QueryKind.Grammar,
                minResults: 1),

            // grammar AND literal
            new QueryCase("%ישראל כי",
                QueryKind.Grammar,
                minResults: 1),

            // grammar with optional char: %שלו?ם — optional ו, then prefix-expand each variant
            new QueryCase("%שלו?ם",
                QueryKind.Grammar,
                minResults: 1),
        };
            ...
            QueryKind    kind)
        {
            ...
            // Fuzzy / wildcard / grammar: use MatchedGroups — the actual expanded terms.
        ...
        private enum QueryKind { Literal, Wildcard, Fuzzy, Grammar }
