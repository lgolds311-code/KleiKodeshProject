...
namespace EverythingSearchClient
{
	...
	internal class MessageReceiverWindow
	{
		...
		{
			Module m = Assembly.GetEntryAssembly() != null
				? Assembly.GetEntryAssembly().ManifestModule
				: typeof(SearchClient).Module;
		...
		internal MessageReceiverWindow()
		{
			...
			// Allow unelevated Everything IPC windows to send data to this potentially elevated window
			ChangeWindowMessageFilterEx(hWnd, WM_COPYDATA, ChangeWindowMessageFilterExAction.Allow, IntPtr.Zero);

			requests.Add(new Tuple<uint, MessageReceiverWindow>((uint)new Random().Next(), this));
		}
		...
		internal bool BuildQuery(string query, SearchClient.SearchFlags flags, uint maxResults, uint offset)
		{
			...
			EverythingIPC.EVERYTHING_IPC_QUERY q = new EverythingIPC.EVERYTHING_IPC_QUERY();
			...
			int querySize = Marshal.SizeOf(typeof(EverythingIPC.EVERYTHING_IPC_QUERY));
			...
			SearchClient.SortDirection sortDirection)
		{
			...
			EverythingIPC.EVERYTHING_IPC_QUERY2 q2 = new EverythingIPC.EVERYTHING_IPC_QUERY2();
			...
			int querySize = Marshal.SizeOf(typeof(EverythingIPC.EVERYTHING_IPC_QUERY2));
			int rawQuerySize = querySize + 2 * (query.Length + 1);
			rawQueryData = new byte[rawQuerySize];
		...
		private static List<Tuple<uint, MessageReceiverWindow>> requests = new List<Tuple<uint, MessageReceiverWindow>>();

		private IntPtr hWnd;
		private byte[] rawQueryData = new byte[0];
		private int queryVersion = 0;

		internal int QueryVersion { get { return queryVersion; } }
		...
		internal bool SendQuery(IntPtr ipcHWnd)
		{
			...
			try
			{
				...
				cdsMem = Marshal.AllocHGlobal(Marshal.SizeOf(typeof(COPYDATASTRUCT)));
		...
		private void ParseResultDataVersion2(IntPtr mem, int size)
		{
			...
			for (int i = 0; i < NumItems; i++)
			{
				...
				if ((requestFlags & EverythingIPC.EVERYTHING_IPC_QUERY2_REQUEST_DATE_CREATED) == EverythingIPC.EVERYTHING_IPC_QUERY2_REQUEST_DATE_CREATED)
				{
					...
					if (timecode > 0)
					{
						try { createDate = DateTime.FromFileTime(timecode); }
						catch { createDate = null; }
					}
					offset += 8;
				}
				...
				{
					long timecode = Marshal.ReadInt64(mem + offset);
					try { modDate = DateTime.FromFileTime(timecode); }
					catch { modDate = null; }
					offset += 8;
				}
