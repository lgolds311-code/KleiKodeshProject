...
const { activeTab, openHebrewBookViewer } = useHebrewBooks();
...
watch(() => activeTab.value?.pdfState, async (pdfState, oldPdfState) => {
  console.log('[HebrewBooksViewPage] PDF state changed:', {
    newState: pdfState,
    oldState: oldPdfState,
    currentPage: activeTab.value?.currentPage
  });
    ...
    if (pdfState.fileUrl) {
      ...
      if (shouldReload) {
        ...
        try {
          ...
          await openHebrewBookViewer(pdfState.bookId, pdfState.bookTitle);
        } catch (error) {
...
watch(() => activeTab.value?.currentPage, (currentPage) => {
