...
class DownloadManager extends BaseDownloadManager {
  async _triggerDownload(blobUrl, originalUrl, filename, isAttachment = false) {
    ...
    // Custom: Use File System Access API for native save dialog when available
    if (blobUrl && window.showSaveFilePicker) {
      try {
        ...
        // User cancelled or API error — fall through to default download
      }
    }
    const a = document.createElement("a");
    a.href = blobUrl;
...
class ViewsManager extends Sidebar {
  ...
  constructor({
    ...
    this.isOpen = false;
    this.active = SidebarView.OUTLINE; // Custom: Default to outline view
...
const PDFViewerApplication = {
  ...
  async run(config) {
    ...
    // Custom: Read filename param for blob URLs (sets document properties + save dialog filename)
