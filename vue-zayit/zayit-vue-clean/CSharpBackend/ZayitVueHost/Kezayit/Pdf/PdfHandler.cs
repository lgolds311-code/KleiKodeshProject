...
namespace Kezayit.Pdf
{
    ...
    public class PdfHandler
    {
        private static readonly string WordCacheDir =
            Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "kezayit", "cache", "word");
        ...
        public async Task HandleRestoreLocalPdf(JsonElement root, string id)
        {
            ...
            _bridge.Reply(id, new { url = "http://kezayit-vue-app/cache/word/" + Path.GetFileName(cached) });
        }
        ...
        private string RegisterFolder(string filePath)
        {
            ...
            if (!_hosts.TryGetValue(folder, out var m))
            {
                string host = "kezayit-pdf-" + (++_hostCounter);
