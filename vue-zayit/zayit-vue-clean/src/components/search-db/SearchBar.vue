...
import { IconSearch20Regular, IconDismiss20Regular, IconFilter20Regular } from '@iconify-prerendered/vue-fluent'
...
<template>
  <BottomSearchBar>
    <template #left>
      <button class="bar-btn" :class="{ 'filter-active': filterCount > 0 }"
        :title="filterCount > 0 ? `סינון: ${filterCount} ספרים` : 'סינון תוצאות'"
        @click.stop="$emit('toggleFilter')">
        <IconFilter20Regular />
      </button>
    </template>
    <input ref="inputRef" v-model="localQuery" type="text" class="search-input"
      placeholder="הזן טקסט לחיפוש..." :disabled="disabled"
      @keydown.enter="handleSearch" @keydown.esc="handleClear" />
    <template #right>
      <button class="bar-btn" :class="{ searching: isSearching }"
...
.ring { width: 20px; height: 20px; animation: spin 1s linear infinite; }
.ring-bg   { stroke: var(--border-color); }
.ring-spin { stroke: var(--accent-color); transform-origin: center; }
.cancel-icon { position: absolute; width: 12px; height: 12px; color: var(--text-secondary); }
