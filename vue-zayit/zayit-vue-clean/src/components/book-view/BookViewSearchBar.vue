...
const placeholder = computed(() => searchMode.value === 'content' ? 'חיפוש בטקסט...' : 'חיפוש במפרשים...')
const matchLabel = computed(() => props.matchCount === 0 ? '0 / 0' : `${props.currentMatch + 1} / ${props.matchCount}`)

const APP_TITLE_BAR = 40, BOOK_TOOLBAR = 32, BAR_WIDTH = 260
function defaultPosition() {
  return { x: window.innerWidth / 2 - BAR_WIDTH / 2, y: APP_TITLE_BAR + (props.toolbarVisible ? BOOK_TOOLBAR : 0) + 4 }
}

const { x, y, style } = useDraggable(barRef, { initialValue: bookViewStore.searchBarPos ?? defaultPosition() })
watch([x, y], ([nx, ny]) => bookViewStore.setSearchBarPos({ x: nx, y: ny }))

function onClose() { inputValue.value = ''; emit('close') }
...
<style scoped>
.search-bar { position: fixed; z-index: 9999; display: flex; align-items: center; gap: 2px; width: fit-content; padding: 5px 6px; background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 8px; box-sizing: border-box; user-select: none; touch-action: none; cursor: grab; box-shadow: 0 4px 16px rgba(0,0,0,0.4), 0 1px 3px rgba(0,0,0,0.25); }
...
.search-inner { display: flex; align-items: center; padding: 5px 6px; gap: 4px; }
...
.match-count { font-size: 11px; color: var(--text-secondary); white-space: nowrap; flex-shrink: 0; min-width: 32px; text-align: end; }
.match-count.no-match { color: #e05252; }
.sep { width: 1px; height: 16px; background: var(--border-color); flex-shrink: 0; margin-inline: 1px; }
.nav-btn, .close-btn { display: flex; align-items: center; justify-content: center; width: 28px; height: 28px; flex-shrink: 0; border-radius: 4px; cursor: pointer; }
