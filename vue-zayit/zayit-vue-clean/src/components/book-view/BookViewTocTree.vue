    ...
    <div ref="panelRef" class="toc-panel">
      ...
      <template v-else>
        <SplitPane :bottom-visible="hasToc && hasAlt" class="toc-body">
          ...
          <template #bottom>
            <BookViewTocTreeSection v-for="section in altTocSections" :key="section.structure.id"
              :title="null" :entries="section.entries" :filter="searchQuery" @select="emit('altSelect', $event)" />
          </template>
        </SplitPane>
...
.toc-state { padding: 32px 16px; text-align: center; font-size: 14px; color: var(--text-secondary); }
.toc-state.error { color: #ff3b30; }
.toc-slide-enter-active, .toc-slide-leave-active { transition: transform 180ms ease; }
