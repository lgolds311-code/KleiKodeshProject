...
const { pause: pauseTyping, resume: resumeTyping } = useIntervalFn(() => {
  ...
  if (charIdx < target.length) { placeholder.value = target.slice(0, ++charIdx) }
  else { pauseTicks = 12; phraseIdx = (phraseIdx + 1) % PLACEHOLDERS.length; charIdx = 0 }
}, 80)

watch(searchQuery, (val) => val ? pauseTyping() : resumeTyping())

function setView(v: 'list' | 'tiles' | 'tree') { view.value = v; tabStore.setBooksView(v) }

onMounted(() => { load(); nextTick(() => searchInputRef.value?.focus()) })
...
<template>
  <div class="books-page">
    <BooksFsTitleBar :view="view" :path="path" :is-searching="isSearching"
      @set-view="setView" @navigate="navigateTo" @reset="fullTreeRef?.reset()" />
