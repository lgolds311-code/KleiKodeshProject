...
**booksCategoryTree.ts** — builds the category tree from flat DB rows, assigns `searchPath` strings to books, and computes period and root-category metadata. `searchPath` is what `useBooksFsSearch` matches against — if you change what's searchable on a book, change it here.
