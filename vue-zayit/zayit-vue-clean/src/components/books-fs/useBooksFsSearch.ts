...
export type TocFsItem = { uid: string; kind: 'toc'; book: BookRow; tocEntryId: number; tocLineIndex: number | null; tocTitle: string; tocPath: string }
...
function stripRoots(rows: TocRow[], bookTitles: Map<number, string>): TocRow[] {
  ...
  return rows.filter(r => !rootIds.has(r.id)).map(r => rootIds.has(r.parentId!) ? { ...r, parentId: null } : r)
}
...
async function matchNodes(nodes: TocSearchNode[], tocWords: string[], bookMap: Map<number, BookRow>): Promise<TocFsItem[]> {
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    if (book) out.push({ uid: `toc-${node.bookId}-${node.id}`, kind: 'toc', book, tocEntryId: node.id, tocLineIndex: node.lineIndex, tocTitle: node.text, tocPath: node.tocDisplayPath })
  }
  return out
}
...
export function useBooksFsSearch(searchQuery: ReturnType<typeof ref<string>>) {
  ...
  watch(debouncedQuery, async (raw) => {
    ...
    if (tocCache?.key === cacheKey) {
      ...
      try { const r = await matchNodes(tocCache.nodes, tocWords, bookMap); if (r.length) results.value = r }
