...
function getSectionEnd(tocEntry: TocEntry, allEntries: TocEntry[]): number {
  ...
  const next = allEntries.slice(idx + 1).find(e => e.lineIndex != null && e.level <= tocEntry.level)
...
export async function findNextTocCommentarySection(
  ...
  const candidates = allEntries.slice(idx + 1).filter(e => e.level === currentTocEntry.level && e.lineIndex != null)
  for (const entry of candidates) {
    const sectionEnd = getSectionEnd(entry, allEntries)
    const rows = await query<{ 1: number }>(SQL.HAS_COMMENTARY_IN_RANGE, [mainBookId, commentaryBookId, entry.lineIndex!, sectionEnd])
...
export async function findPrevTocCommentarySection(
  ...
  const idx = allEntries.indexOf(currentTocEntry)
  const candidates = allEntries.slice(0, idx).filter(e => e.level === currentTocEntry.level && e.lineIndex != null).reverse()
  for (const entry of candidates) {
    const sectionEnd = getSectionEnd(entry, allEntries)
    const rows = await query<{ 1: number }>(SQL.HAS_COMMENTARY_IN_RANGE, [mainBookId, commentaryBookId, entry.lineIndex!, sectionEnd])
    if (rows.length) return entry
