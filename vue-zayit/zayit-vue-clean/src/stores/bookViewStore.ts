...
import { ZOOM_CONFIG, zoomIn as zoomInUtil, zoomOut as zoomOutUtil, resetZoom as resetZoomUtil } from '@/composables/useZoom'
...
export const useBookViewStore = defineStore('bookView', () => {
  ...
  function toggleBottomPanel() { toggleBottomPanelSignal.value++ }
  ...
  function zoomIn()    { zoom.value = zoomInUtil(zoom.value);    idbSet(KEYS.SETTINGS_ZOOM, zoom.value) }
  function zoomOut()   { zoom.value = zoomOutUtil(zoom.value);   idbSet(KEYS.SETTINGS_ZOOM, zoom.value) }
  function resetZoom() { zoom.value = resetZoomUtil();           idbSet(KEYS.SETTINGS_ZOOM, zoom.value) }

  return { toolbarVisible, toggleBottomPanelSignal, toggleBottomPanel, searchBarPos, isBookViewActive, zoom, init, toggleToolbar, setSearchBarPos, zoomIn, zoomOut, resetZoom }
})
