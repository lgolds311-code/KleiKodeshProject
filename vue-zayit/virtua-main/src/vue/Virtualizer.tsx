...
export interface VirtualizerHandle {
  ...
  /**
   * Get current scroll position with item index and pixel offset into that item.
   * @param element The element at the top of the viewport (found by caller using elementFromPoint)
   * @returns Object with index (item at viewport top) and offset (pixels scrolled into that item)
   */
  getScrollPosition(element: Element): { index: number; offset: number };
  /**
  ...
  /**
   * Scroll to a specific position within an item.
   * @param element The element to scroll to (must have data-index attribute)
   * @param offsetIntoItem pixel offset into the item (how many pixels scrolled into that item)
   */
  scrollToPosition(element: Element, offsetIntoItem: number): void;
  /**
  ...
  setup(props, { emit, expose, slots }) {
    ...
    expose({
      ...
      getScrollPosition: (element: Element) => {
        // Calculate offset of given element relative to scroll container
        const container = containerRef.value;
        if (!container || !container.parentElement || !element) {
      ...
      scrollToPosition: (element: Element, offsetIntoItem: number) => {
        // Get index from element
        const index = parseInt(element.getAttribute('data-index') || '0', 10);
        
        const container = containerRef.value;
        if (!container || !container.parentElement) {
          console.warn('[Virtua] scrollToPosition: No container or parent');
          return;
        }

        const scrollContainer = container.parentElement;
        
        console.log('[Virtua] scrollToPosition START:', {
          index,
          offsetIntoItem,
          currentScrollTop: scrollContainer.scrollTop
        });
        
        // Step 1: Scroll to index to ensure element is rendered
        scroller.$scrollToIndex(index, { align: 'start' });
        
        // Step 2: Wait for scroll to complete, then apply offset adjustment
        // Use multiple RAF to ensure DOM is fully settled
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              ...
              const adjustment = currentOffset - offsetIntoItem;
              
              console.log('[Virtua] scrollToPosition ADJUSTMENT:', {
                currentOffset,
                targetOffset: offsetIntoItem,
                adjustment,
              ...
              if (Math.abs(adjustment) > 0.5) {
                if (isHorizontal) {
                  scrollContainer.scrollLeft -= adjustment;
                } else {
                  scrollContainer.scrollTop -= adjustment;
                }
                
                console.log('[Virtua] scrollToPosition APPLIED:', {
