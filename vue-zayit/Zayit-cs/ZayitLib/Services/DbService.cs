...
namespace Zayit.Services
{
    public class DbService
    {
        ...
        public object GetTree(string cq, string bq) =>
            new { categoriesFlat = _db.ExecuteQuery(cq), booksFlat = _db.ExecuteQuery(bq) };

        public object GetConnectionTypes(string q) => _db.ExecuteQuery(q);

        public object GetToc(int bookId, string q) => new { tocEntriesFlat = _db.ExecuteQuery(q) };
        ...
        public object GetLineRange(int bookId, int start, int end, string q) => _db.ExecuteQuery(q);

        public object GetLinks(int lineId, string tabId, int bookId, string q, object[] p = null) =>
            _db.ExecuteQuery(q, p);
