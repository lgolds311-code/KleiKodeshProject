...
namespace Zayit.Viewer
{
    public class HebrewBooksDownloadManager
    {
        ...
        public HebrewBooksDownloadManager(CoreWebView2 webView, WebView2 webViewControl = null)
        {
            _webView = webView;
        ...
        public async Task PrepareHebrewBookDownload(string bookId, string title, string action)
        {
            try
            {
                ...
                if (action == "download")
                {
                    ...
                    {
                        // Use async dialog helper to prevent freezing
                        // Set the book title as the default filename
                        string defaultFileName = $"{SanitizeFileName(title)}.pdf";
                        string initialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
                        
                        selectedPath = await WebViewDialogHelper.ShowSaveFileDialogAsync(
                            _webViewControl,
                            "PDF Files (*.pdf)|*.pdf",
                            "Save Hebrew Book",
                            defaultFileName,
                    ...
                    {
                        // Fallback to synchronous dialog if WebView2 control not available
                        using (var dialog = new SaveFileDialog())
                        {
                            dialog.Filter = "PDF Files (*.pdf)|*.pdf";
                            dialog.Title = "Save Hebrew Book";
                            dialog.FileName = $"{SanitizeFileName(title)}.pdf";
                    ...
                    Console.WriteLine($"[HebrewBooks] User selected save path: {selectedPath}");
