...
const { linkGroups, isLoading, commentaryBookIds, loadCommentaryLinks: loadLinks } = useCommentaryLoader()
...
// Filters composable
const { selectedCategoryFilter, filteredGroupOptions, availableCategories } = useCommentaryFilters(
    sortedLinkGroups,
    selectedConnectionTypeId,
    computed(() => commentaryBookIds.value),
    computed(() => props.book)
)
...
function handleCategoryFilterChange(category: string | null) {
    ...
    // Check if selected category is a connection type (not a commentary period)
    const connectionTypeNames = ['מקור', 'תרגומים', 'קשרים', 'שונות']
    
    if (category && connectionTypeNames.includes(category)) {
        // Map Hebrew label back to connection type name
        ...
        const connectionTypeName = nameMap[category]
        if (connectionTypeName) {
            const connectionTypeId = connectionTypesStore.getConnectionTypeId(connectionTypeName)
            if (connectionTypeId) {
                selectedConnectionTypeId.value = connectionTypeId
                selectedCategoryFilter.value = null // Clear period filter when switching connection type
                return
            }
        }
    }
    
    // Otherwise, it's a commentary period filter
