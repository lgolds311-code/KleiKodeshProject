...
├── uses: useBookViewPage.ts, useBookViewer.ts
...
│       └── uses: useLineViewToolbarActions.ts, useToolbarPosition.ts
└── TocTreeView.vue (TOC panel)
    ├── uses: useToc.ts
...
├── CommentaryViewToolbar.vue (child - toolbar)
├── CommentaryContentView.vue (child - content display)
│   └── uses: useCommentaryContentView.ts, useCommentarySearch.ts
...
HebrewBooksViewPage.vue (page - single book view)
└── uses: useHebrewBooks.ts
```
