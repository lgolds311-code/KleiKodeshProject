...
namespace KleiKodesh
{
    ...
    public class RegexFind : RegexFindBase
    {
        ...
        public void Execute()
        {
            ...
            Results = GetResults();
            if ((Mode == SearchMode.Forward || Mode == SearchMode.Back) && Results.Length > 0)
            {
        ...
        public void ReplaceCurrent()
        {
            if (Results == null || Results.Length == 0)
                return;
            var currentPos = SelectionStart;
            var currentResult = Results.FirstOrDefault(r => r.Start == currentPos);
            if (currentResult == null)
                return;
            var rng = currentResult.Range;
            string replacementText = Regex.Replace(rng.Text, _effectivePattern, Replace.Text);
            rng.Text = replacementText;
            // Apply formatting
            if (Replace.Bold) rng.Font.Bold = 1; else rng.Font.Bold = 0;
