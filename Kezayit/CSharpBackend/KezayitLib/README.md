# KezayitLib — C# Backend for the Kezayit Vue App

A .NET class library that hosts the Kezayit Vue app inside a WebView2 control and bridges it to native Windows APIs (file system, SQLite, PDF handling, HebrewBooks downloads, Bloom filter search).

## How It Integrates with the VSTO Add-in

`KleiKodeshVsto` instantiates `KezayitLib.AppViewer` (a WinForms `UserControl`) and passes it to `TaskpaneManager.Show()`. The task pane hosts the control, which in turn owns the WebView2 instance and all backend handlers.
...
└── Settings/
    └── AppSettings.cs        — Registry-backed settings for the Kezayit app
```
...
2. Maps virtual host `kezayit-vue-app` → `kezayit/` folder (the built Vue app).
3. Injects `JsBridge.Script` so the page has `window.__webviewAction` available before any JS runs.
4. Navigates to `https://kezayit-vue-app/index.html`.
