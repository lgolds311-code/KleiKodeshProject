...
namespace FtsLib.Core
{
    ...
    ///
    /// Search algorithms are provided by SearchExecutor — the same algorithms
    ...
    internal sealed class RamIndex : Dictionary<string, RamIndexEntry>
    {
        ...
        public IEnumerable<int> Search(IEnumerable<string> terms) =>
            SearchExecutor.AndSearch(terms, GetIterator, GetCount);

        // ── OR ───────────────────────────────────────────────────────

        public IEnumerable<int> SearchOr(IEnumerable<string> terms) =>
            SearchExecutor.OrSearch(terms, GetIterator);

        // ── Mixed AND/OR ─────────────────────────────────────────────

        public IEnumerable<int> Search(IEnumerable<IEnumerable<string>> groups) =>
            SearchExecutor.MixedSearch(groups, GetIterator);
    }
}
