...
// Insert source rows
const insertSource = dst.prepare('INSERT OR IGNORE INTO source (label, lang, url) VALUES (?, ?, NULL)')
for (const label of Object.values(SOURCE_LABELS)) insertSource.run(label, 'ארמית')
...
const insertSense = dst.prepare(`
  INSERT OR IGNORE INTO sense (headword, nikud, etymology, cross_ref, period_tag, source_id, sense_order)
  VALUES (?, ?, ?, ?, NULL, ?, ?)
`)
