...
Pinia stores. The only layer (besides `idbPersistence.ts`) allowed to read from or write to IndexedDB. Components and composables never import from `idbPersistence.ts` directly — they go through a store.
...
**themeStore** — active theme preset and reading background color. Use this to read or change the current theme.
