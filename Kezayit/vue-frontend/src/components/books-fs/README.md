...
**useBooksFsSearch.ts** — two-tier search. Phase 1 matches books by word-prefix matching against each book's `searchPath` (title + category path), sorted by match quality then tree order. Phase 2 kicks in when no books match: splits the query into a book part and a TOC part, fetches TOC entries for candidate books, and searches them with `matchWords` from `tocSearchUtils.ts`. Do not add fuzzy logic to the TOC phase — it uses exact matching by design.
